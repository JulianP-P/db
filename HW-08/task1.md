...
|Настройки                |По умолчанию|Pgconfigurator|Мое предложение| Старое значение |Комментарии|
|:------------------------|:-----------|:-------------|---------------|-----------------|-----------|
|shared_buffers           |128 MB      |1024 MB      | 4 GB | 128 MB | Буфер для разделяемой памяти рекомендуется брать в 25% от ОЗУ |
|work_mem                 |4 MB        |32 MB        | 70 MB (по формуле Total RAM * 0,25 / max_connections) | 4 MB | Память, которая будет используется при обработке запросов. Выделяется для каждого процесса. |
|maintenance_work_mem     |64 MB       |320 MB       | 1 GB | 64 MB | Максимальный объём памяти для операций VACUUM, CREATE INDEX и ALTER TABLE ADD FOREIGN KEY |
|huge_pages               |try         |off          | off (в нашей бд нет огромных таблиц + в докере для использования этой функции необходимо провести доп исследования о возможности) | try | Запрашивание огромных таблиц из общей памяти отключено|
|effective_cache_size     |4 GB        |11 GB        | 12 GB (рекомендация - 50%-75% от ОЗУ) | 4 GB | Оценка памяти, доступной для кэширования диска, рекомендуется брать в 50-75% от ОЗУ |
|effective_io_concurrency |1           |100          | 200 (рекомендации для ssd) | 1 | Число одновременных операций ввода-вывода. Параметр зависит от типа диска. При SSD стоит выбирать значения в несколько сотен|
|random_page_cost         |4           |1.25         | 1 | 4 | Стоимость чтения одной произвольной страницы с диска. Параметр зависит от типа диска. При SSD стоит выбирать значения ближе к 1|
|tps                      | -          |-            | - | - | - |
