...
|work_mem                 |4 MB        |32 MB         |17476 kB|70 MB (по формуле Total RAM * 0,25 / max_connections) |Память, которая будет используется при обработке запросов. Выделяется для каждого процесса. |
...
|effective_cache_size     |4 GB        |11 GB         |12 GB   |12 GB (рекомендация - 50%-75% от ОЗУ) | Оценка памяти, доступной для кэширования диска, рекомендуется брать в 50-75% от ОЗУ |
|effective_io_concurrency |1           |100           |200     |200 (рекомендации для ssd) | Число одновременных операций ввода-вывода. Параметр зависит от типа диска. При SSD стоит выбирать значения в несколько сотен|
